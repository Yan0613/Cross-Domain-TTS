    ...
    for epoch in range(max_func_call):
        ...
        if args.use_vllm:
            ...
            gen_output = llm.generate(prompts, SamplingParams(
                                ...
                                seed=args.seed,
                                n=1, stop=stop_words))
            output_responses = [gen_output[index].outputs[0].text for index in range(len((prompts)))]
    ...
    for epoch in range(max_func_call):
        ...
        if args.use_vllm:
            ...
            for index in range(len(prompts)):
                # if not gen_output[index].outputs[0].text.endswith(tuple(stop_words)):
            ...
            while 1:
                ...
                # assert len(active_traj) == len(prompts_)
